...
export const SequenceComponentStatusD: Decoder<SequenceComponentStatus> = D.type({
  seqCompId: ComponentIdD,
  sequencerLocation: D.array(AkkaLocationD)
})

const AgentStatusD: Decoder<AgentStatus> = D.type({
  agentId: ComponentIdD,
  seqCompsStatus: D.array(SequenceComponentStatusD)
})

const AgentStatusSuccessD: Decoder<AgentStatusSuccess> = D.type({
