...
const done:Done = await teventService.publish(event)
//#publish
const dd = async () => {
//#get
  const sourcePrefix = new Prefix('IRIS', 'component')
...
//#subscribe
  ...
  const subscription: Subscription = eventService.subscribe(eventKeys, 10)(onEventCallback)
...
//#p-subscribe
  ...
  const subscription: Subscription = eventService.pSubscribe('ESW', 10,'.*')(onEventCallback)
