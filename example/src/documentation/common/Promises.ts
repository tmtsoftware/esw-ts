import {
  CommandService, ComponentId, Prefix, Setup, SubmitResponse
...
const dd = async () => {
  ...
  CommandService(componentId)
    .then(commandService => {
      ...
      responsePromise.then(submitResponse1 => {
        handleResponse(submitResponse1);
        commandService.submit(setup2)
          .then(submitResponse2 => {
            handleResponse(submitResponse2);
            commandService.query(submitResponse2.runId)
              .then(fetchNewESWState);
          });
      }).catch(handleError);
