...
Config service has following [APIs](#apis):

|  API                                      | Input args                          | Returns               |
| ----------------------------------------- | ----------------------------------- | --------------------- |
| [create](#create)                         | path, configData, annex, comment    | ConfigId              |
| [update](#update)                         | path, configData, comment           | ConfigId              |
| [getActive](#getactive)                   | path                                | Option< ConfigData >  |
| [getLatest](#getlatest)                   | path                                | Option< ConfigData >  |
| [getById](#getbyid)                       | path, configId                      | Option< ConfigData >  |
| [getByTime](#getbytime)                   | path, time                          | Option< ConfigData >  |
| [exists](#exists)                         | path, id                            | boolean               |
| [delete](#delete)                         | path, comment                       | void                  |
| [list](#list)                             | fileType, pattern                   | ConfigFileInfo[ ]     |
| [history](#history)                       | path, from, to, maxResults          | ConfigFileRevision[ ] |
| [historyActive](#historyactive)           | path, from, to, maxResults          | ConfigFileRevision[ ] |
| [setActiveVersion](#setactiveversion)     | path, id, comment                   | void                  |
| [resetActiveVersion](#resetactiveversion) | path, comment                       | void                  |
| [getActiveByTime](#getactivebytime)       | path, time                          | Option< ConfigData >  |
| [getActiveVersion](#getactiveversion)     | path                                | Option< ConfigId >    |
| [getMetadata](#getmetadata)               |                                     | ConfigMetadata        |

## Rules and Checks
1. The config file path must not contain !#<>$%&'@^``~+,;= or any whitespace character
2. If the input file is > 10MB or has lot of non ASCII characters, then for optimization, server will archive it in annex store.
3. Large and binary files can be forced to go to the ‘annex’ store by using a annex=true flag in the create operation.
...
#### Examples to create client for Config service
