...
export class Angle {
  ...
  constructor(readonly uas: number) {}
  ...
  private static formatSecs(sec: number): string {
    ...
    else if (Angle.isNear(sec, 0.01)) return `${sec.toFixed(2)}`
    else return `${sec.toFixed(3)}`
  }
  ...
  static parseDe(deSign: string, deDegree: string, deMin: string, deSec: string | null): Angle {
    ...
    const deg = Number(deDegree)
    if (deg < 0 || deg > 89) throw 'Invalid deDegree: ' + deg
    const min = deMin != null ? Number(deMin) : Number(0)
    if (min < 0 || min >= 60) throw 'Invalid deMin: ' + min
    const sec = deSec != null ? Number(deSec) : Number(0)
    if (sec < 0 || sec >= 60) throw 'Invalid deSec: ' + sec

    return new Angle(sign * (deg * Number(Angle.D2Uas) + min * Number(Angle.M2Uas) + sec * Number(Angle.S2Uas)))
  }
  ...
  static parseRa(raHour: string, raMin: string, raSec: string | null): Angle {
    const raHour2 = Number(raHour)
    if (raHour2 < 0 || raHour2 > 23) throw 'Invalid raHour: ' + raHour2
    const raMin2 = raMin != null ? Number(raMin) : Number(0)
    if (raMin2 < 0 || raMin2 >= 60) throw 'Invalid raMin: ' + raMin2
    const raSec2 = raSec != null ? Number(raSec) : Number(0)
    if (raSec2 < 0 || raSec2 >= 60) throw 'Invalid raSec: ' + raSec2

    return new Angle(raHour2 * Number(Angle.H2Uas) + raMin2 * Number(Angle.HMin2Uas) + raSec2 * Number(Angle.HSec2Uas))
  }
  ...
  static parseRaFromString(ra: string): Angle {
    ...
    if (ar3 && ar3.length == 4) {
      ...
      return Angle.fromDegree(Number(d))
        .add(Angle.fromArcMinute(Number(m)))
        .add(Angle.fromArcSec(Number(s)))
    }
  ...
  static assertRa(ra: number) {
    if (ra < 0 || ra >= Math.PI * 2) throw 'Invalid RA: ' + ra
  }

  static assertDe(de: number) {
    if (de < -Angle.D2R * 90 || de > Angle.D2R * 90) throw 'Invalid DE: ' + de
  }
}
